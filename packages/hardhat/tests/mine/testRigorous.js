...
async function mineRig(rigAddress, miner, rigRecipient = null, index = 0) {
  const rigContract = await ethers.getContractAt("MineRig", rigAddress);
...
describe("Rigorous Tests", function () {
  ...
  describe("Edge Cases and Boundary Conditions", function () {
    it("Mining at exact epoch boundary", async function () {
      ...
      const rigContract = await ethers.getContractAt("MineRig", result.rig);
  ...
  describe("Full Lifecycle Integration", function () {
    ...
    it("Multiple independent rigs operate correctly", async function () {
      ...
      const rig1Contract = await ethers.getContractAt("MineRig", rig1.rig);
      const rig2Contract = await ethers.getContractAt("MineRig", rig2.rig);
    ...
    it("Rig state survives long idle period", async function () {
      ...
      expect(state.nextUps).to.be.gt(0);
